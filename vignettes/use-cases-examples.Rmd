---
title: "Use Cases and Examples for disaggR"
...
The `disaggR` package enables statistically sound disaggregation of aggregate data with uncertainty propagation. When disaggregating data (splitting a total into components), the components are naturally correlated. These correlations must be properly accounted for in uncertainty analysis to avoid mis-estimating uncertainty in downstream calculations.

This vignette demonstrates how to use `disaggR` through a practical example in Industrial Ecology.
...
### Generating Correlated Samples with disaggR
...
The `disaggR` package provides a simple interface for generating statistically valid samples of disaggregated data while properly accounting for correlations. This is crucial for accurate uncertainty propagation in subsequent analyses.
